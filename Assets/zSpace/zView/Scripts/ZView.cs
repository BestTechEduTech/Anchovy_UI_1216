        ...
        void OnDrawGizmos()
        {
            ...
            if (this.ARModeShowMask)
            {
                Camera mainCamera = GameObject
                    .FindObjectOfType<VirtualCameraAR>().CoreCamera;

                if (mainCamera != null)
                {
                    if(mainCamera.stereoTargetEye != StereoTargetEyeMask.Both)
                        mainCamera = mainCamera.transform.parent.GetComponent<Camera>();

                    Gizmos.color = Color.white;
                    Gizmos.matrix = mainCamera.transform.parent.localToWorldMatrix;
